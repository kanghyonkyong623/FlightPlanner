...
from map.CreateVectorLayer.Qgsnewvectorlayerdialog import QgsNewVectorLayerDialog
import define
...
#         if self.actionShowOverview.isChecked():
    ...
    def addVectorLayer(self):
        # createVectorLayerDlg = Qgsnewvectorlayerdialog(self.mView)
        resultStr = QgsNewVectorLayerDialog.runAndCreateLayer(self.mView)
            
    def createContextMenu(self):
        menu = QMenu()
        actionAddGroup = QgisHelper.createAction(menu, "&Add GroupLayer", self.addGroup)
        actionAddVectorLayer = QgisHelper.createAction(menu, "&Add VectorLayer", self.addVectorLayer)
