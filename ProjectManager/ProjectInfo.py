...
class ProjectList:
    ...
    def Find(self, strName, type=None):
        try:
            if type is None:
                ...
                i = 0
                for pi in self.ProjectsList:
                    if (pi.FullName == strName) and pi.Pt == type:
                        return i
                    i += 1
                return None
        except:
    ...
    def GetListOfWsOrProcedure(self, type, projectName, subProjectName, workspaceName=None):
        ...
        if type == enumProjectType.ptWorkspace:
            for projectInfo in self.ProjectsList:
                if projectInfo.Pt != enumProjectType.ptWorkspace:
                    continue
                if projectInfo.ProjName == projectName:
                    if projectInfo.SubProjName == subProjectName:
                        resultList.append(projectInfo.WorkspaceName)
        elif type == enumProjectType.ptFile:
            for projectInfo in self.ProjectsList:
                if projectInfo.Pt != enumProjectType.ptFile:
                    continue
                if projectInfo.ProjName == projectName:
                    if projectInfo.SubProjName == subProjectName:
                        if projectInfo.WorkspaceName == workspaceName:
    ...
    def GetIndexOfWsOrProcedure(self, type, projectName, subProjectName, workspaceName=None, procedureName=None):
        if type == enumProjectType.ptSubProject:
            i = 0
            for projectInfo in self.ProjectsList:
                if projectInfo.Pt != enumProjectType.ptSubProject:
                    i += 1
                    continue
                if projectInfo.ProjName == projectName:
                    if projectInfo.SubProjName == subProjectName:
                        return i
                i += 1
        elif type == enumProjectType.ptWorkspace:
            i = 0
            for projectInfo in self.ProjectsList:
                if projectInfo.Pt != enumProjectType.ptWorkspace:
                    i += 1
            ...
            for projectInfo in self.ProjectsList:
                ...
                if projectInfo.ProjName == projectName:
                    if projectInfo.SubProjName == subProjectName:
                        if projectInfo.WorkspaceName == workspaceName:
                            if projectInfo.WorkspaceName == workspaceName:
                                if projectInfo.ProcedureName == procedureName:
                                    return i
                i += 1
